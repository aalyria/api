...
class SpacetimeCallCredentials(grpc.AuthMetadataPlugin):
  ...
  def _exchange_proxy_auth_jwt_for_oidc_token(self, proxy_auth_jwt_value):
    ...
    connection = http.client.HTTPSConnection(self.gcp_oidc_token_creation_host)
  ...
  def is_oidc_token_expired(self, now):
    return (not self.oidc_token or
            now + SpacetimeCallCredentials.OIDC_TOKEN_EXPIRATION_TIME_MARGIN
